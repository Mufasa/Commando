...
class option(object):
    ...
    @property
    def shortName(self):
        return self.__options['shortName'] if 'shortName' in self.__options else None

    @property
    def default(self):
        return self.__options['default'] if 'default' in self.__options else None
...
class Cli(object):
    ...
    def __validateShortNames(cls, supportedOptions):
        ...
        for option in supportedOptions.values():
            if option.hasShortName:
                if type(option.shortName) != type(''):
                ...
                elif option.shortName in optionsWithShortNames:
                    ...
                    if 'is' + methodSuffix in supportedOptions:
                        matchingLongNameOption = supportedOptions['is' + methodSuffix]
                    elif 'get' + methodSuffix in supportedOptions:
    ...
    def __validatePositionalArguments(cls, supportedOptions):
        ...
        for option in supportedOptions.values():
            if option.hasPosition:
                if option.position in positionalArguments:
    ...
    def __constructHelpText(cls, prog, purpose, options, positionalArguments):
        ...
        if maxShortNameLength > 0:
            formatString = '%-{longNameLength}s%-{shortNameLength}s %-{valueLength}s'.format(longNameLength=maxLongNameLength + 2, shortNameLength=maxShortNameLength, valueLength=maxValueLength)
        else:
            formatString = '%-{longNameLength}s%s %-{valueLength}s'.format(longNameLength=maxLongNameLength, valueLength=maxValueLength)
        ...
        if maxDefaultValueLength > 0:
            formatString += '%-{defaultValueLength}s'.format(defaultValueLength=defaultValueLength)
        else:
            formatString += '%s'
        ...
        for option in options.values() + positionalArguments:
            ...
            helpText = formatString % (longName, shortName, helpTextComponents['value'], defaultValue)
...
class _ParsedOptions(object):
    ...
    def __init__(self, optionsClass, helpText, options, positionalArguments, args):
        ...
        setattr(self.__optionsInstance, 'helpText', helpText)
        for methodName in self.__options.keys():
            optionDescription = self.__options[methodName]
            if optionDescription.name in parsedOptions:
...
class _Context(object):
    ...
    def addOption(self, optionName):
        ...
        if 'get' + methodNameSuffix in self.__options:
    ...
    def addShortOption(self, optionName):
        ...
        if optionName in self.__shortOptions:
